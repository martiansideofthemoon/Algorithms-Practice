...
# you need to.
same_identity = IF(AND(OR('male (?x)','female (?x)')),
            THEN('same_identity (?x) (?x)'))
brother = IF(AND('parent (?x) (?y)',
                'parent (?x) (?z)',
                'male (?y)',
                NOT('same_identity (?y) (?z)')),
            THEN('brother (?y) (?z)'))
sister = IF(AND('parent (?x) (?y)',
                'parent (?x) (?z)',
                'female (?y)',
                NOT('same_identity (?y) (?z)')),
            THEN('sister (?y) (?z)'))
...
daughter = IF(AND('parent (?x) (?y)',
                'female (?y)'),
            THEN('daughter (?y) (?x)'))
grandrelation = IF(AND('parent (?x) (?y)',
                'parent (?y) (?z)'),
            THEN('grandparent (?x) (?z)',
                  'grandchild (?z) (?x)'))
cousin = IF(AND(OR('brother (?x) (?y)',
                   'sister (?x) (?y)'),
                    'parent (?x) (?z)',
                    'parent (?y) (?w)'),
            THEN('cousin (?z) (?w)',
...
# family_rules.
family_rules = [same_identity,brother,sister,father,mother,son,daughter,grandrelation,cousin]                    # fill me in
...
WHAT_I_FOUND_INTERESTING = 'dsfsd'
